...
class GoogleMap : SupportMapFragment() {
     ...
     */
    suspend fun addMarker(vararg markerParam: Marker) {
        val latLng = mutableListOf<LatLng>()
        for (i in markerParam) {
            latLng.add(i.latLng)
            val marker = MarkerOptions()
                .position(i.latLng)
                .title(i.title)
            if (i.icon != null) {
                val mIcon = BitmapDescriptorFactory.fromBitmap(i.icon!!)
                marker.icon(mIcon)
            }
     ...
     */
    private fun newMarkersIsAdded(latLng: MutableList<LatLng>) {
        moveCamera(latLng)
    }
    ...
    fun moveCamera(vararg latLng: LatLng) {
        ...
        for (i in latLng) {
            Log.i("asd", "${i.latitude}  ${i.longitude}")
    ...
    fun moveCamera(latLng: MutableList<LatLng>) {
        val latLongBoundBuilder = LatLngBounds.builder()
        for (i in latLng) {
            Log.i("asd", "${i.latitude}  ${i.longitude}")
            latLongBoundBuilder.include(i)
    ...
    fun addPolyLine(polyline: PolylineOptions){
        Handler(Looper.getMainLooper()).post {
